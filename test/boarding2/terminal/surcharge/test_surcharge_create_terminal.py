...
@pytest.mark.parametrize('enable, percent', [(True, random_surcharge_percent(5)), (False, None)])
def test_create_terminal_surcharge_valid(enable, percent):
    ...
    create_terminal_response = wn.create_terminal(LocalMerchant.GO.itemid, original_terminal, TsysSaratogaTerminal, key)
    assert_that(create_terminal_response, instance_of(TsysSaratogaTerminal))
    assert_that(create_terminal_response.terminal_features.surcharge, equal_to(original_terminal.terminal_features.surcharge))
...
def test_create_terminal_surcharge_empty():
    key = db.get_api_key(GO_KEY)
    original_terminal = wn.get_terminal(LocalMerchant.GO.itemid, TERM_ID, TsysSaratogaTerminal, key)
    original_terminal.secret = 'someSecretPhrase'
...
def test_create_terminal_surcharge_percent_validation(percent, error_message):
    key = db.get_api_key(GO_KEY)
    original_terminal = wn.get_terminal(LocalMerchant.GO.itemid, TERM_ID, TsysSaratogaTerminal, key)
    original_terminal.secret = 'someSecretPhrase'
    ...
    create_terminal_response = wn.create_terminal(LocalMerchant.GO.itemid, original_terminal, TsysSaratogaTerminal, key)
    assert_that(create_terminal_response, instance_of(Error))
    assert_that(create_terminal_response.details[0].error_message, equal_to(error_message))
    assert_that(create_terminal_response.details[0].source._property, equal_to('percent'))
